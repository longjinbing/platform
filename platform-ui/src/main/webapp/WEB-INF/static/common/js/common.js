var ZtreeInput = Vue.extend({
    ...
    template: '<input v-model="id"  name="parentId" class="input-hidden" type="text">' +
        '<div @click="showList()" class="input-group">' +
        '<input class="form-control" disabled v-model="name" placeholder="请选择" type="text">' +
        '<span id="select-button" class="input-group-addon btn-select">选择</span>' +
        '</div>',
    methods: {
        showList: function () {
            var _this = this;
            ...
            if ($ztree.length > 0) {
                ...
                var obj = $("#select-button");
                $ztree = $('<ul id="' + id + '" class="select-ztree  ztree"></ul>');
                if ($("#" + id).length > 0) {
                    $("#" + id).remove();
                }
                ...
                var e = obj.parent().parent().append($ztree);
                var setting = {
                    data: {
                        simpleData: {
                            enable: true
                ...
                $.get(transUrl(this.url), function (r) {
                    var zNodes = r.data;
                    var zTreeObj = $.fn.zTree.init($ztree, setting, zNodes);
                })

                function selectZTreeClick(treeId, treeNode, clickFlag) {
                    var group = $("#" + treeId).parent().children("div");
                    group.children("input[type=hidden]").val(treeNode.id).change();
                    group.children("input[type=text]").val(treeNode.name).change();
                    $("#" + treeId).hide();
                    _this.$emit('change', {
                        id: treeNode.id,
                        name: treeNode.name
...
var orderNum = Vue.extend({
    props: {
        num: 0
    },
    template: '<div class="input-group">' +
        '<span @click="change(-1)" class="input-group-addon">-</span>' +
        '<input v-model="num" class="form-control order" name="orderNum" type="text">' +
        '<span @click="change(+1)" class="input-group-addon">+</span>' +
        '</div>',
    methods: {
    ...
    watch:{
        num:function (value) {
            ...
            this.$emit("orderchange", this.num);
        }
    }
});
var statusRadio = Vue.extend({
    props: {
        status:1
    },
    template:'<div class="status-wrap">' +
        '<label>' +
        '<input value="1" v-model="status" name="status" type="radio">启用</label>' +
        '<label>' +
        '<input value="0" v-model="status" name="status" type="radio">禁用</label>'+
        '</div>',
    watch:{
...
var PidInput = Vue.extend({
    props: {
        id: 0,
        field:'',
        name: ''
    },
    template: '<input v-model="id"  :name="field" class="input-hidden" type="text">' +
        '<input class="form-control" disabled v-model=name" type="text">'
});

Vue.component('ztree-input', ZtreeInput);
Vue.component("order", orderNum);
Vue.component("status-radio", statusRadio);
Vue.component("pid-input", PidInput);
$(function () {
            ...
            if (value != undefined && value.length > 0) {
                ...
                $.get(transUrl(infoUrl) + value, function (res) {
                    for (var key in res.data) {
                        if (key == "name") {
                            obj.next().children("input[type=text]").val(res.data[key]);
                        }
                    }
...
function check(r) {
    if (r.code == 405) {
        alert(r.msg);
        setTimeout(function () {
            window.top.location.href = transUrl("/account/unauthorized?message=")+r.msg;
        }, 0)
        return false;
    } else if (r.code == 403) {
        top.layer.msg(r.msg);
        showError(r.data);
    }else if (r.code == 402||r.code==404) {
        top.layer.msg(r.msg)
    } else if (r.code == 0) {
        return true;
    } else if (r.code == -1) {
...
function topLoginWindow() {
    if (window != top.window && window.location.href.indexOf("/account/") > -1) {
...
function showError(data) {
    $.each(data, function (key, value) {
        var label = $("#" + key + "-error");
        if (label.length > 0 && label.hasClass("validate-success")) {
            label.removeClass("validate-success").addClass("error").html(value);
        }
    })
}
...
function overflowHidden(value, num) {
    ...
    if (value != undefined) {
        if (value.length > num) {
            return '<span data-toggle="tooltip"  title="' + value + '">' + value.toString().substring(0, num) + '...</>';
        } else {
...
function errorMessageFormat(value) {
    if (value != undefined) {
        return "<label class=\"error\">" + value + "</label>";
    }
}

function clearVailator() {
    $(".validate-success").remove();
    validator.resetForm();
}

function transSysLogLevel(value) {
    if (value == 1) {
        return '<span class="btn btn-default btn-xs">普通错误</span>';
    } else if (value == 2) {
        return '<span class="btn btn-info btn-xs">一般错误</span>';
    } else if(value==3) {
        return '<span class="btn btn-danger btn-xs">未知错误</span>';
    }else if(value==4) {
        return '<span class="btn btn-danger btn-xs">急需修复错误</span>';
    }else {
        return '<span class="btn btn-default btn-xs">运行错误</span>';
    }
};

function transSysLogStaus(value) {
    if (value == undefined||value == 0) {
        return '<span class="btn btn-danger btn-xs">待处理</span>';
    } else if (value == 1) {
        return '<span class="btn btn-success btn-xs">处理完成</span>';
    }
};
