		...
		methods : {
                ...
                if (id != '') {
                    ...
                    if(row[0].type==1) {
                        ...
                        top.layer.msg('评价已经结束了，不能再次开启！');
                            ...
                            var url=transUrl("/comment/start");
                            $.ajax({
                                type : "POST",
                                url : url,
            ...
            progressdetails:function(){
                ...
                if (id != '') {
                    ...
                    if(row[0].status==1){
                       ...
                       newtab("/course/resultdetails/"+id,"课程评价进度");
                    }
                }
			},
            progress:function(){
			...
			details:function(){
				var id = getSelectedRow();
				if (id != '') {
					var url = transUrl("/comment/details/")+id;
					$.get(url, function(r) {
						if (check(r)) {
							vue.comment= r.data;
			...
			deleteData : function() {
				...
				if (id != '') {
					confirm('确定要删除选中的记录？', function() {
						var url=transUrl("/comment/delete");
						$.ajax({
							type : "POST",
							url : url,
							contentType : "application/json",
			...
			saveOrUpdate : function() {
				var url = vue.comment.id == null ? transUrl("/comment/save") : transUrl("/comment/update");
				$.ajax({
					type : "POST",
					url : url,
					contentType : "application/json",
	...
	var validator;
	$(function() {
		var url=transUrl("/comment/list?pid=")+vue.courseId;
